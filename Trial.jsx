...
const Carousel = () => {
  ...
  const fetchCarousels = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://Ndush.pythonanywhere.com/carousels');
  ...
  const uploadCarousel = async () => {
    ...
    try {
      setUploadProgress(0);
      const response = await axios.post('https://Ndush.pythonanywhere.com/upload', formData, {
  ...
  const toggleCarouselStatus = async (imageId, currentStatus) => {
    try {
      await axios.put(`https://Ndush.pythonanywhere.com/carousel/toggle/${imageId}`);
      fetchCarousels();
    } catch (err) {
  ...
  const deleteCarousel = async (imageId) => {
    if (window.confirm('Are you sure you want to delete this carousel image?')) {
      try {
        await axios.delete(`https://Ndush.pythonanywhere.com/delete/${imageId}`);
        fetchCarousels();
      } catch (err) {
